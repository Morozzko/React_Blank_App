  ...
  if (Array.isArray(props.highlight)) {
        ...
        {acc.map((part, i) => {
          return (
            <span
              key={i}
              style={
                Array.isArray(props.highlight) && props.highlight.find(ele => part.toLowerCase().match(ele.toLowerCase()))
                  ? {
                      color: part.match(new RegExp('(' + props.errorRegEx + ')', 'gi'))
                        ? props.errorColor
                          ? props.errorColor
                          : 'blue'
                        : props.color
    ...
    return (
      <span>
        {parts.map((part, i) => {
          return (
            <span key={i} style={part.toLowerCase().match(props.highlight.toString().toLowerCase()) ? { color: 'red' } : {}}>
              {part}
            </span>
          );
        })}
