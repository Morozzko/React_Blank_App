const { curry, lensProp, lensPath, over, prepend, append, findIndex, remove, assocPath } = require('ramda')
...
const prependWebpackPlugin = curry((plugin, config) => over(pluginsLens, prepend(plugin), config))

const appendWebpackPlugin = curry((plugin, config) => over(pluginsLens, append(plugin), config))

const getPluginsNames = config => config.plugins.map(ele => ele.constructor.name)
...
const replaceWebpackPlugin = curry((replacement, constructorName, config) =>
  injectPluginIndex(constructorName, i => assocPath(['plugins', i], replacement, config), config)
)

const removeWebpackPlugin = curry((constructorName, config) =>
  injectPluginIndex(constructorName, i => over(pluginsLens, remove(i, 1), config), config)
)
