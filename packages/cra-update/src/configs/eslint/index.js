const { both, prop, propSatisfies, includes, reduce, assocPath, dissocPath, keys, type } = require('ramda')
...
const useDotRc = config => assocPath(['options', 'useEslintrc'], true, clearDefaults(config))

const useConfigFile = (options, config) =>
  reduce(
    (stage, key) => assocPath(['options', 'baseConfig', key], options[key], stage),
  ...
  edit(
    subConfig => {
      ...
      switch (sourceType) {
        ...
        default: {
          error(`use-eslint-config expects argument of type 'String' or 'Object' but received ${sourceType}`)
        }
      }
    },
